...
from GayuMusic import YouTube, app
from GayuMusic.core.call import Gayu
from GayuMusic.misc import SUDOERS, db
from GayuMusic.utils.database import (
    is_active_chat,
    is_music_playing,
    is_muted,
    is_nonadmin_chat,
...
from GayuMusic.utils.decorators.language import languageCB
from GayuMusic.utils.formatters import seconds_to_min
from GayuMusic.utils.inline import (
    close_markup,
    panel_markup_1,
    panel_markup_2,
    panel_markup_3,
...
from GayuMusic.utils.inline.play import stream_markup
from GayuMusic.utils.stream.autoclear import auto_clean
from GayuMusic.utils.thumbnails import get_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Gayu.pause_stream(chat_id)
        buttons = [
            [
                InlineKeyboardButton(
        ...
        await Gayu.resume_stream(chat_id)
        buttons_resume = [
            [
                InlineKeyboardButton(
        ...
        await Gayu.mute_stream(chat_id)
        ...
        await Gayu.unmute_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await Gayu.st_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await Gayu.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await Gayu.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_7"])
            ...
            try:
                await Gayu.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_7"])
            button = stream_markup2(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await Gayu.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_7"])
            if videoid == "telegram":
                button = stream_markup2(_, chat_id)
